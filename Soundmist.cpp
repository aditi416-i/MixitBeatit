...
void sort()
{
	...
	for (int i = 0; i < n - 1; i++)
		...
		{
			f.seekg(j * sizeof(music));
			f.read((char *)&s, sizeof(music));
			f.read((char *)&a, sizeof(music));
			if (strcmpi(s.gn(), a.gn()) > 0)
			{
				f.seekp(j * sizeof(music));
				f.write((char *)&a, sizeof(music));
				f.write((char *)&s, sizeof(music));
			}
		}
	f.close();
}
...
int main()
{
	...
	system("cls");
	{
		cout << endl
			 ...
			 << endl
			 << endl
			 << setw(57) << ".______~WELCOME TO Mix it Beat it~ _______." << endl
			 << endl
			 << endl;
